...
**`cc status` ask-surface contract (2026-08-26):**
...
- cc-dispatch `handoff.py` keys `hud_up` off `status.hud.up`. That field no longer means “overlay process on :3334”.
...
- Mutating AX commands (`mode`, `new-task`, `inject`, `recent --pick`) refuse unless `team_active`. `inspect` / `status` / `hud-ask` / `afk-*` stay ungated. Opt-out: `--no-afk-guard` or `CC_SKIP_AFK_GUARD=1` (tests / emergency only).
- Default `cc inject` preserves a real composer draft (clipboard + relay notify). `--clobber` restores overwrite. `--cowork-safe` is a no-op kept for documented callers.
